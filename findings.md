...
| User name | root |
| AWS access key | ASIA************IUSC |
| Source IP address | xx.xx.xxx.xx |
...
- **Identity:** The action was performed by the account `root` user (not an IAM user or assumed role), account ID `************`, using a temporary session (`accessKeyId` begins with `ASIA`, indicating STS-issued credentials rather than long-term root keys).
...
- **Source IP:** `xx.xx.xxx.xx`, consistent with the operator's own network at the time of the action.
